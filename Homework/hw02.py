...
def pingpong(n):
    ...
    def helper(k, value, up):
        if k == n:
            return value
        elif k % 7 == 0 or num_sevens(k)!=0:
            if up:
                return helper(k + 1, value - 1, False)
            else:
                return helper(k + 1, value + 1, True)
        else:
            if up:
                return helper(k + 1, value + 1, True)
            else:
...
def count_change(amount):
    ...
    def count(number, at_most):
        if number < 0:
            ...
            return count(number - pow(2, at_most), at_most) + count(number, at_most - 1)
    ...
    at_most = find_largest_power(amount)

    return count(amount, at_most)
